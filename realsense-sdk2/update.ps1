...
function Get-VersionFromUrlPath($url) {
    return (Split-Path ( Split-Path $url ) -Leaf).Substring(1)
}
...
# Extracts the version from the file, using the pattern below, if theres no version in the file, then return from the path.
    $pathVersion = Get-VersionFromUrlPath $url
...
function Get-ReleaseNotesUrl($version) {
    $version = (Select-String -InputObject $version -Pattern '((\d+\.){2}\d)').Matches.Groups[1].Value
    return 'https://github.com/IntelRealSense/librealsense/releases/tag/v' + $version
}
